...
# --- Callback Functions for Instant UI Updates ---
def handle_add(uid, ticker):
    """Callback to add a stock. Updates DB and session state."""
...
def handle_remove(uid, ticker):
    """Callback to remove a stock. Updates DB and session state."""
...
# --- Helper Functions (normalize_prices, display_sentiment_bar) ---
...
def display_sentiment_bar(container, score):
    ...
    container.markdown(f"""
        <div style="width: 100%; background-color: #ddd; border-radius: 5px; height: 24px;">
            <div style="width: {bar_width}%; background-color: {sentiment_color}; height: 24px; border-radius: 5px;"></div>
        </div>
    """, unsafe_allow_html=True)

def display_stock_details(container, ticker_data):
    """Displays the detailed data for a single stock using on_click callbacks."""
    ...
    # Use on_click to call the new handler functions
    if is_in_watchlist:
    ...
    with sub_tabs[0]:
        if ticker_data['hist'].empty: st.warning("No historical price data for the selected range.")
        else:
            c1, c2 = st.columns(2)
...
with st.sidebar:
    ...
    # Load the watchlist when the page first loads
...
if analyze_button:
    if start_date >= end_date:
        ...
        # Load watchlist into session state if it's not there
        if 'watchlist' not in st.session_state:
            st.session_state.watchlist = get_watchlist(st.session_state.get('uid'))
        ...
        if not tickers:
            ...
            # (The data fetching and tab creation logic is the same...)
            all_data = []
            ...
            with main_tabs[0]: # Performance Tab
                ...
                else: st.info("No historical data to plot for the selected range.")

            with main_tabs[1]: # AI Insights Tab
                if len(all_data) == 2:
                    with st.spinner("Generating AI comparison..."): ai_comp = get_ai_comparison(all_data[0], all_data[1], investor_level)
                    st.markdown(ai_comp)
                else:
                    with st.spinner("Generating AI summary..."): ai_sum = get_ai_summary(all_data[0]['news'], all_data[0]['ticker'], investor_level)
                    st.markdown(ai_sum)

            with main_tabs[2]: # Detailed Analysis Tab
                if len(all_data) == 1: display_stock_details(st, all_data[0])
                elif len(all_data) == 2:
                    col1, col2 = st.columns(2)
                    display_stock_details(col1, all_data[0]); display_stock_details(col2, all_data[1])
